...
class Proxy_systems extends Table_systems
{
    ...
    {
        $q = 'SELECT * FROM systems WHERE system_version_id = ? AND system_sector = ? AND system_x = ? AND system_y = ?';
        $a = array($versionId, $systemSector, $systemX, $systemY);
        $row = $this->fetchRow($q, $a);
        if (!$row) {
            $row = new Row_systems();
            $row->system_sector = $systemSector;
            $row->system_version_id = $versionId;
